...
#define CREATE_STRING(str_ptr,TEXT)                { (str_ptr) = new string(TEXT); acdxVp1ASTStringCollector.push_back(str_ptr); }
...
list<string*> acdxVp1LexSymbolCollector;

list<string*> acdxVp1ASTStringCollector;
...
#include "ACDXVP1Flex.gen" 

int acdxVp1StartParse(void *ptr)
{  
    yyparse(ptr);
    
    list<string*>::iterator iter = acdxVp1LexSymbolCollector.begin();
    
    while(iter != acdxVp1LexSymbolCollector.end())
    {   
        //delete (*iter);
        iter++;
    }
    
    acdxVp1LexSymbolCollector.clear();

    iter = acdxVp1ASTStringCollector.begin();
    
    while(iter != acdxVp1ASTStringCollector.end())
    {   
        //delete (*iter);
        iter++;
    }
    
    acdxVp1ASTStringCollector.clear();
