...
private:
    ...
    mutable Matrixf matrixState[MATRICES_SIZE];
...
public:
    ...
    void setMatrix(MatrixId mid, GLuint unit, const Matrixf& mat);
    void setMatrix(const Matrixf& data, GLuint id);

    /* Interface for getting a matrix easily */
    const Matrixf& getMatrix(MatrixId mid, MatrixType mtype, GLuint unit = 0) const;
    
    /* Compiler interface for getting a matrix */
    const Matrixf& getMatrix(GLuint id) const;    
