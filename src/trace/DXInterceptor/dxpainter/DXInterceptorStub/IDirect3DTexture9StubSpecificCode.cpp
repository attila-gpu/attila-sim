...
IDirect3DTexture9InterceptorStub::IDirect3DTexture9InterceptorStub(DXPainter* painter, IDirect3DTexture9* original, IDirect3DDevice9InterceptorStub* creator) : 
...
IDirect3DTexture9InterceptorStub::~IDirect3DTexture9InterceptorStub()
{
  if (m_locks)
  {
    delete[] m_locks;
...
HRESULT IDirect3DTexture9InterceptorStub::DoSpecific(DXMethodCallPtr call)
{
  switch (call->GetToken())
  {
  case DXMethodCallHelper::TOK_IDirect3DTexture9_QueryInterface:
    {
      IID param1;
  ...
  case DXMethodCallHelper::TOK_IDirect3DTexture9_AddRef:
    {
  ...
  case DXMethodCallHelper::TOK_IDirect3DTexture9_Release:
    {
  ...
  case DXMethodCallHelper::TOK_IDirect3DTexture9_GetDevice:
    {
  ...
  case DXMethodCallHelper::TOK_IDirect3DTexture9_GetSurfaceLevel:
    {
      UINT param1;
      ...
      HRESULT hr = m_original->GetSurfaceLevel(param1, &pSurface);
      ...
      {
        new IDirect3DSurface9InterceptorStub(m_painter, param1, pSurface, m_creator);
      }
      else
      {
        pRefCounter->Release();
  ...
  case DXMethodCallHelper::TOK_IDirect3DTexture9_LockRect:
    {
      UINT param1;
      CHECK_CALL(call->Pop_UINT(&param1));
      ...
      RECT* param3Ptr = &param3;
      {
        ...
        {
          CHECK_CALL(call->Pop_RECT(param3Ptr));
        }
      }
      ...
      HRESULT hr = m_original->LockRect(param1, &lockedRect, param3Ptr, param4);

      if (FAILED(hr) || (param4 & D3DLOCK_READONLY))
      {
        lockedRect.pBits = 0;
        m_locks[param1].SetLockedRect(&lockedRect);
      }
      else
      {
        m_locks[param1].SetLockedRect(&lockedRect);
  ...
  case DXMethodCallHelper::TOK_IDirect3DTexture9_UnlockRect:
    {
      UINT param1;
      CHECK_CALL(call->Pop_UINT(&param1));
      ...
      HRESULT hr = m_original->UnlockRect(param1);
      CHECK_CALL_RETURN_VALUE_HRESULT(hr);
    }
    break;
  
  case DXMethodCallHelper::TOK_IDirect3DTexture9_GetPrivateData:
    return D3DResource9_GetPrivateData(m_original, call);
    break;
  
  case DXMethodCallHelper::TOK_IDirect3DTexture9_SetPrivateData:
    return D3DResource9_SetPrivateData(m_original, call);
    break;
  
  case DXMethodCallHelper::TOK_IDirect3DTexture9_FreePrivateData:
