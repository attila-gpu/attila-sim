...
private:
    ...
    acd_int* indexes0; ///< RBank register indexes 0
    acd_int* indexes1; ///< RBank register indexes 1
    acd_bool* used; ///< bitmap of registers used
    acd_uint count; ///< number of registers within the RBank
...
*/
     ...
     */
    RBank(acd_uint size, const std::string& name="");
     ...
     */
    QuadReg<T>& get(acd_uint pos) const;
     ...
     */
    QuadReg<T>& operator[](acd_uint pos);
     ...
     */
    void set(acd_uint pos, const QuadReg<T>& value);
     ...
     */
    RType getType(acd_uint pos, acd_int& index0, acd_int& index1) const;
     ...
     */
    void setType(acd_uint pos, RType type = QR_UNKNOWN, acd_int index0 = 0, acd_int index1 = 0);
     ...
     */
    acd_uint getRegister(acd_bool& found, const QuadReg<T>& value, RType regType = QR_UNKNOWN, 
...
template<class T>
RBank<T>::RBank(acd_uint size, const std::string& name) : name(name)
{
    count = size;
    bank = new QuadReg<T>[count];
    types = new RType[count];
...
template<class T>
acd_uint RBank<T>::size() const
{
    return count;
}

template<class T>
void RBank<T>::setSize( acd_uint size_ )
{
    if ( count != 0 )
    {
        delete[] bank;
...
template<class T>
QuadReg<T>& RBank<T>::get(acd_uint pos) const
{
...
template<class T>
QuadReg<T>& RBank<T>::operator[](acd_uint pos)
{
    /*  Check index to the register bank.  */
    if (pos >= count)
...
template<class T>
void RBank<T>::set(acd_uint pos, const QuadReg<T>& value)
{
    /*  Check index to the register bank.  */
    if (pos >= count)
...
template<class T>
void RBank<T>::clear(acd_uint pos)
{
    /*  Check index to the register bank.  */
    if (pos >= count)
...
template<class T>
RType RBank<T>::getType(acd_uint pos,int& index0, int& index1) const
{
    /*  Check index to the register bank.  */
    if (pos >= count)
...
template<class T>
void RBank<T>::setType(acd_uint pos, RType type, int index0, int index1)
{
    /*  Check index to the register bank.  */
    if (pos >= count)
...
template<class T>
acd_uint RBank<T>::getRegister(bool& found, const QuadReg<T>& value, RType regType, int index0, int index1) const
{
    acd_uint i;
    found = false;
    for ( i = 0; i < count; i++ )
...
template<class T>
bool RBank<T>::usedPosition(acd_uint pos) const
{
    if (pos >= count)
...
template<class T>
acd_uint RBank<T>::findFree() const
{
    u32bit i;
    for ( i = 0; i < count; i++ )
        if ( !used[i] )
...
public:
    ...
    GPUBank(acd_uint size, std::string name) : RBank<T>(size,name), readPorts(0) {}

    GPUBank(acd_uint size, acd_uint readPorts = 0, std::string name="") :
        RBank<T>(size,name), readPorts(readPorts)
        {}

    GPUBank(const RBank<T>& rb, acd_uint readPorts = 0, std::string name="") :
