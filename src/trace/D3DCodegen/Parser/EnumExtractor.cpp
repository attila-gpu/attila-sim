...
vector<EnumDescription>& EnumExtractor::GetEnums()
{
  return *m_lstEnums;
}
...
bool EnumExtractor::ParseEnumMember(EnumDescription& edes, string& cadena)
{
  ...
  if (br.matched)
  {
    ...
    if (test.length() == 0)
    {
      EnumDescriptionMember member;
      member.name = results.backref(1).str();
      Utilities::String::TrimString(member.name);
