...
%token <token_atrib> TOK_ARBFP10_HEADER TOK_OPTION TOK_PARAM TOK_ATTRIB TOK_TEMP TOK_OUTPUT TOK_ALIAS TOK_ABS
 TOK_ADD TOK_CMP TOK_COS TOK_DP3 TOK_DP4 TOK_DPH TOK_DST TOK_EX2 TOK_FLR TOK_FRC TOK_KIL TOK_KLS TOK_CMPKIL TOK_CHS
 TOK_ZXP TOK_ZXS TOK_LG2 TOK_LIT TOK_LRP TOK_MAD TOK_MAX TOK_MIN TOK_MOV TOK_MUL TOK_FXMUL TOK_FXMAD TOK_FXMAD2 TOK_POW 
 TOK_RCP TOK_RSQ TOK_SIN TOK_SCS TOK_SGE TOK_SLT TOK_SUB TOK_SWZ TOK_TEX TOK_TXB TOK_TXP TOK_XPD TOK_ABS_SAT TOK_ADD_SAT 
 TOK_CMP_SAT TOK_COS_SAT TOK_DP3_SAT TOK_DP4_SAT TOK_DPH_SAT TOK_DST_SAT TOK_EX2_SAT TOK_FLR_SAT TOK_FRC_SAT TOK_LG2_SAT 
 TOK_LIT_SAT TOK_LRP_SAT TOK_MAD_SAT TOK_MAX_SAT TOK_MIN_SAT TOK_MOV_SAT TOK_MUL_SAT TOK_POW_SAT TOK_RCP_SAT TOK_RSQ_SAT 
 TOK_SIN_SAT TOK_SCS_SAT TOK_SGE_SAT TOK_SLT_SAT TOK_SUB_SAT TOK_SWZ_SAT TOK_TEX_SAT TOK_TXB_SAT TOK_TXP_SAT TOK_XPD_SAT 
 TOK_PROGRAM TOK_RESULT TOK_STATE TOK_FRAGMENT TOK_TEXTURE TOK_SAMPLE TOK_COLOR TOK_PRIMARY TOK_SECONDARY TOK_TEXCOORD 
 TOK_FOGCOORD TOK_POSITION TOK_ATTRIB_M TOK_ENV TOK_LOCAL TOK_MATERIAL TOK_AMBIENT TOK_DIFFUSE TOK_SPECULAR TOK_EMISSION 
 TOK_SHININESS TOK_FRONT TOK_BACK TOK_LIGHT TOK_SPOT TOK_DIRECTION TOK_HALF TOK_ATTENUATION TOK_LIGHTMODEL 
...
%type <irinstruction> instruction ALUInstruction TexInstruction
                      VECTORop_instruction VECTORop SCALARop_instruction SCALARop 
                      BINSCop_instruction BINSCop BINop_instruction BINop 
                      TRIop_instruction TRIop CHS_Instruction
%type <irsamplinstr> SAMPLE_instruction SAMPLEop
%type <irkillinstr> KIL_instruction
%type <irzxpinstr> ZXP_instruction
...
/**********************************************************************************************/
    ...
    ;
    CHS_Instruction       : TOK_CHS
        {
            $$ = new IRCHSInstruction($1.linea,string($1.texto->c_str()));
        }
    ; 
    ...
    KIL_instruction      : TOK_KIL vectorSrcReg
        {
            $$ = new IRKillInstruction($1.linea,string($1.texto->c_str()));
            $2->setLine($$->getLine());
            $$->setSource0($2);
        }
		                     | TOK_KLS vectorSrcReg ',' TOK_SAMPLE '[' integer ']'
        {
			$$ = new IRKillInstruction($1.linea,string($1.texto->c_str()));
            $2->setLine($$->getLine());
            $$->setSource0($2);
            $$->setIsKillSampleInstr();
            $$->setKillSample($6);
        }
    ;
    ZXP_instruction      : TOK_ZXP vectorSrcReg
        {
            $$ = new IRZExportInstruction($1.linea,string($1.texto->c_str()));
            $2->setLine($$->getLine());
            $$->setSource0($2);
        }
		                     | TOK_ZXS vectorSrcReg ',' TOK_SAMPLE '[' integer ']'
        {
			$$ = new IRZExportInstruction($1.linea,string($1.texto->c_str()));
            $2->setLine($$->getLine());
            $$->setSource0($2);
