...
int CRollupCtrl::_InsertPage(const char* caption, CDialog* pwndTemplate, int idx, BOOL bAutoDestroyTpl)
{
  ...
  // Disable themes for the groupbox and the button
  ThemeHelperST themeHelper;
  if (themeHelper.IsAppThemed())
  {
    themeHelper.DisableWindowTheme(groupbox->m_hWnd);
    themeHelper.DisableWindowTheme(but->m_hWnd);
  }
...
LRESULT CALLBACK CRollupCtrl::DlgWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (_this->m_PageHeight>r.Height())	//Can Scroll ?
	{
		...
		switch (uMsg) {
			...
			case WM_SETCURSOR:
			{
				if ((HWND)wParam==hWnd)	{ ::SetCursor(AfxGetApp()->LoadCursor(IDC_CURSOR_HAND)); return TRUE; }
			break;
			}
...
void CRollupCtrl::OnPaint() 
{
	...
	dc.FillSolidRect(&br, RC_SCROLLBARCOLOR_FG);
	dc.FillSolidRect(CRect(br.left,2,br.right,br.top), RC_SCROLLBARCOLOR_BK);
	dc.FillSolidRect(CRect(br.left,br.bottom,br.right,2+ClientHeight), RC_SCROLLBARCOLOR_BK);
...
void CRollupCtrl::OnContextMenu( CWnd* /*pWnd*/, CPoint pos )
{
	...
	if (m_cmenuCtxt.CreatePopupMenu())
	{
		...
		m_cmenuCtxt.AppendMenu(MF_STRING,		RC_MID_EXPANDALL,		"Open all pages"	);
		m_cmenuCtxt.AppendMenu(MF_STRING,		RC_MID_COLLAPSEALL,	"Close all pages"	);
