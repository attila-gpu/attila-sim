...
//
//  Modified:   7/August/2003 by Hans Dietrich  (see //+++hd comments)
...
{
	TRACE(_T("in CButtonST::OnRButtonUp\n"));
...
{
	UINT nNewType = (UINT) (wParam & SS_TYPEMASK);
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	Redraw(pDC, lpDIS->itemState, rcItem, FALSE);
...
void CButtonST::Redraw(CDC *pDC, UINT itemState, CRect& rcItem, BOOL bHoverCancel)
{
	...
	if (m_bIsCheckBox)
	{
		m_bIsPressed  =  (itemState & ODS_SELECTED) || (m_nCheck != 0);
	} // if
	else	// Normal button OR other button style ...
	{
		m_bIsPressed = (itemState & ODS_SELECTED);
	...
	m_bIsFocused  = (itemState & ODS_FOCUS);
	m_bIsDisabled = (itemState & ODS_DISABLED);

	CRect itemRect = rcItem;
	...
	else
	{
		if (bHoverCancel)			//+++hd
		{
			COLORREF crColor = m_crColors[BTNST_COLOR_BK_OUT];
			CBrush brBackground(crColor);
			pDC->FillRect(&itemRect, &brBackground);
		}
		else
		{
			OnDrawBackground(pDC, &itemRect);
	...
	if (m_csIcons[0].hIcon)
	{
		DrawTheIcon(pDC, !sTitle.IsEmpty(), &rcItem, &captionRect, m_bIsPressed, m_bIsDisabled);
	} // if

	if (m_csBitmaps[0].hBitmap)
	{
		pDC->SetBkColor(RGB(255,255,255));
		DrawTheBitmap(pDC, !sTitle.IsEmpty(), &rcItem, &captionRect, m_bIsPressed, m_bIsDisabled);
	} // if
	...
	if (sTitle.IsEmpty() == FALSE)
	{
		...
		DrawTheText(pDC, (LPCTSTR)sTitle, &rcItem, &captionRect, m_bIsPressed, m_bIsDisabled);
