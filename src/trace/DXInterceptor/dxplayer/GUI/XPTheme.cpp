...
//
// Heavily based on the CVisualStyleXP class of David Yuheng Zhao. The
// difference is that the dll is global and the HTHEME handle is wrapped. The
// HTHEME handle is automatically released as the object is deleted. All API
// functions not directly related to a HTHEME instance are realized as static
// methods. To optimize the execution speed, static function pointers are used.
// During execution, either the actual function or the failure function is
// assigned to the static function pointer, which is used from there on.
//
// To explicitly Release the dll, call the static Release method. To check
// whether Theming is available, that is, if the platform is XP or newer, use
// the static method IsAvailable
//
