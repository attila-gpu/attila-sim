...
MacroExpander::MacroExpander(vector<CppMacro>& macros, bool verbose) :
m_lstMacros(macros),
m_verbose(verbose)
{
}
...
void MacroExpander::MatchMacro(CppMacro& macro, string& cadena)
{
  ...
  match_results::backref_type br1 = patro_macro1.match(macro.Left(), results1);
  if (br1.matched)
  {
    string macroName;
    ...
    if (macro.Left().find('(') != string::npos)
    {
    ...
    if (macroApplied)
    {
      ...
      if (macroApplied)
      {
        ...
        // la cadena final. [
        string macroPatro2 = macro.Right();
    ...
    if (m_verbose)
    {
      if (macroApplied)
      {
        cout << "Macro applied '" << macro.Left() << "'" << endl;
      }
      else
      {
        cout << "Macro NOT applied '" << macro.Left() << "'" << endl;
      }
    }
  }
  else
  {
    DXCodeGenException e("macro '" + macro.Left() + "' could'nt be parsed");
    throw e;
  }
}
