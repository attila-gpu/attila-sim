...
vector<StructDescription>& StructExtractor::GetStructs()
{
  return *m_lstStructs;
}
...
bool StructExtractor::ParseStructMember(StructDescription& sdes, string& cadena)
{
  ...
  if (br.matched)
  {
    ...
    if (test.length() == 0)
    {
      StructDescriptionMember smember;

      smember.type = results.backref(1).str();
      smember.name = results.backref(3).str();

      Utilities::String::TrimString(smember.type);
      Utilities::String::TrimString(smember.name);
