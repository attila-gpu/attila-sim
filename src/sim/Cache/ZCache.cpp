...
u32bit ZCache::cacheCounter = 0;


/*  Z cache constructor.  */
ZCache::ZCache(u32bit ways, u32bit lines, u32bit stampsLine,
        u32bit bytesStamp, u32bit readP, u32bit writeP, u32bit pWidth, u32bit reqQSize, u32bit inReqs,
        u32bit outReqs, bool zComprDisabled, u32bit maxZBlocks, u32bit blocksPerCycle,
...

{
    ...
    GPU_ASSERT(
        if (ways == 0)
            panic("ZCache", "ZCache", "At least one ways required.");
        if (lines == 0)
            panic("ZCache", "ZCache", "At least one cache line per ways required.");
        if (lineStamps == 0)
            panic("ZCache", "ZCache", "At least one stamp per cache line required.");
        if (stampBytes < (STAMP_FRAGMENTS * 1))
            panic("ZCache", "ZCache", "At least one byte per fragment in the stamp.");
        if (GPU_MOD(stampBytes, STAMP_FRAGMENTS) != 0)
            panic("ZCache", "ZCache", "Bytes per stamp must be a multiple of the fragments in the stamp.");
        if (readPorts == 0)
            panic("ZCache", "ZCache", "At least one read port required.");
        if (writePorts == 0)
            panic("ZCache", "ZCache", "At least one read port required.");
        if (portWidth == 0)
            panic("ZCache", "ZCache", "Port width must be at least one byte.");
        if (inputRequests == 0)
            panic("ZCache", "ZCache", "Input request queue must have at least one entry.");
        if (outputRequests == 0)
        ...
        if (maxBlocks < ((MAX_DISPLAY_RES_X * MAX_DISPLAY_RES_Y) / (lineStamps * STAMP_FRAGMENTS)))
            panic("ZCache", "ZCache", "Block state memory not enough large for largest supported resolution.");
        if (blocksCycle == 0)
            panic("ZCache", "ZCache", "At lest one block state entry should be cleared per cycle.");
        if (comprLatency == 0)
            panic("ZCache", "ZCache", "Compression unit latency must be at least 1.");
        if (decomprLatency == 0)
            panic("ZCache", "ZCache", "Decompression unit latency must be at least 1.");
    )
    ...
    GPU_ASSERT(
        if (inputBuffer == NULL)
            panic("ZCache", "ZCache", "Error allocating input buffers.");
    )
    ...
    GPU_ASSERT(
        if (outputBuffer == NULL)
            panic("ZCache", "ZCache", "Error allocating output buffers.");
    )
    ...
    GPU_ASSERT(
        if (maskBuffer == NULL)
            panic("ZCache", "ZCache", "Error allocating mask buffers.");
    )
    ...
    GPU_ASSERT(
        if (readQueue == NULL)
            panic("ZCache", "ZCache", "Error allocating the read queue.");
    )
    ...
    GPU_ASSERT(
        if (writeQueue == NULL)
            panic("ZCache", "ZCache", "Error allocating the write queue.");
    )
    ...
    for(i = 0; i < inputRequests; i++)
    {
        ...
        GPU_ASSERT(
            if (inputBuffer[i] == NULL)
                panic("ZCache", "ZCache", "Error allocating input buffer.");
        )
    }
    ...
    for(i = 0; i < outputRequests; i++)
    {
        ...
        GPU_ASSERT(
            if (outputBuffer[i] == NULL)
                panic("ZCache", "ZCache", "Error allocating output buffer.");
        )
        ...
        GPU_ASSERT(
            if (maskBuffer[i] == NULL)
                panic("ZCache", "ZCache", "Error allocating mask buffer.");
        )
    }
    ...
    GPU_ASSERT(
        if (blockState == NULL)
            panic("ZCache", "ZCache", "Error allocating the block state information.");
    )
    ...
    GPU_ASSERT(
        if (writeCycles == NULL)
            panic("ZCache", "ZCache", "Error allocating write port cycle counters.");
    )
    ...
    GPU_ASSERT(
        if (writeCycles == NULL)
            panic("ZCache", "ZCache", "Error allocating read port cycle counters.");
    )
...
bool ZCache::fetch(u32bit address, u32bit &way, u32bit &line, DynamicObject *source)
{
...
bool ZCache::allocate(u32bit address, u32bit &way, u32bit &line, DynamicObject *source)
{
    u32bit i;
    u32bit e;
    u32bit block;
...
bool ZCache::read(u32bit address, u32bit way, u32bit line, u8bit *data)
{
    u32bit i;
...
bool ZCache::write(u32bit address, u32bit way, u32bit line, u8bit *data,
    bool *mask)
{
    u32bit i;
...
void ZCache::unreserve(u32bit way, u32bit line)
{
...
void ZCache::processMemoryTransaction(MemoryTransaction *memTrans)
{
    ...
    switch(memTrans->getCommand())
    {
        case MT_READ_DATA:
            ...
            GPU_ASSERT(
                if (memoryCycles > 0)
                    panic("ZCache", "processMemoryTransaction", "Memory bus still busy.");
            )
        ...
        default:
            ...
            panic("ZCache", "processMemoryTransaction", "Unsopported transaction type.");
            break;
    }
}
...
MemoryTransaction *ZCache::update(u64bit cycle, MemState memState)
{
...
/*  Clears the Z cache.  */
bool ZCache::clear(u32bit depth, u8bit stencil)
{
    /*  Reset the cache.  */
    if (clearMode)
    {
...
void ZCache::swap(u32bit bufferAddress)
{
...
/*  Check HZ updates.  */
bool ZCache::updateHZ(u32bit &block, u32bit &z)
{
...
void ZCache::clock(u64bit cycle)
{
    u32bit i, e;
    u32bit readEndReq;
            ...
            GPU_ASSERT(
                    ...
                    i++, e = GPU_PMOD(e - 1, inputRequests))
                {
                    ...
                    if (readQueue[e].address == cacheRequest->outAddress)
                    {
                        ...
                        panic("ZCache", "clock", "Writing a line not fully read from the cache.");
                    }
                }
            )
    ...
    if (inputs > 0)
    {
        ...
        switch(blockState[readQueue[nextInput].block])
        {
            ...
            default:
                ...
                panic("ZCache", "clock", "Unsupported color block state mode.");
        }
    }
...
//printf("ZCache -> clear block -> clearStencil %x clearDepth %x clear %x\n", clearStencil, clearDepth, (clearStencil << 24) | (clearDepth & 0x00ffffff));
                    ...
                    for(i = 0; i < lineSize; i += 4)
                        *((u32bit *) &inputBuffer[nextRead][i]) = (clearStencil << 24) | (clearDepth & 0x00ffffff);
                ...
                default:
                    ...
                    panic("ZCache", "clock", "Unsupported block mode.");
                    break;
            }
    ...
    if ((writeOutputs > 0) && (compressCycles == 0))
    {
        ...
        FragmentOpEmulator::blockMinMaxZ((u32bit *) outputBuffer[nextWrite], lineStamps * 4, minZ, maxZ, min, max);
        ...
        switch(comprMode)
        {
            ...
            default:
                panic("ZCache", "clock", "Unsupported compression mode.");
                break;
        }
...
MemoryTransaction *ZCache::requestBlock(u32bit readReq)
{
    u32bit size;
    u32bit offset;
    MemoryTransaction *memTrans;
...
/*  Requests a compressed block/line of data.  */
MemoryTransaction *ZCache::writeBlock(u32bit writeReq)
{
    u32bit size;
    u32bit offset;
    MemoryTransaction *memTrans;
...
void ZCache::copyBlockStateMemory(ZBlockState *buffer, u32bit blocks)
{
    GPU_ASSERT(
        if (blocks > maxBlocks)
...
u32bit ZCache::address2block(u32bit address)
{
    u32bit block;
