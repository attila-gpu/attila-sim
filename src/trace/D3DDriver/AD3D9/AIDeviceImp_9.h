...
#ifndef AIDEVICE_9_H_INCLUDED
#define AIDEVICE_9_H_INCLUDED
...
class AIDeviceImp9 : public IDirect3DDevice9{

public:

    /// Singleton method, maintained to allow unimplemented methods to return valid interface addresses.
    static AIDeviceImp9 &getInstance();

    AIDeviceImp9(AIDirect3DImp9* i_parent, D3DPRESENT_PARAMETERS * pPresentationParameters);
    ~AIDeviceImp9();
...
private:
    ...
    AIDirect3DImp9* i_parent;

    ULONG refs;

    set<AIVertexBufferImp9*> i_vertex_buffer_childs;
    set<AIIndexBufferImp9*> i_index_buffer_childs;
    set<AIVertexDeclarationImp9*> i_vertex_declaration_childs;
    set<AIVertexShaderImp9*> i_vertex_shader_childs;
    set<AIPixelShaderImp9*> i_pixel_shader_childs;
    set<AITextureImp9*> i_texture_childs;
    set<AICubeTextureImp9*> i_cubetexture_childs;
    set<AIVolumeTextureImp9*> i_volumetexture_childs;
    set<AISurfaceImp9*> i_surface_childs;


    //  Stores the current state.
    map<D3DRENDERSTATETYPE, DWORD> currentRenderState;
    
    //  Stores the captured state.
    map<D3DRENDERSTATETYPE, DWORD> capturedRenderState;
...
public:
    ...
    void captureRenderState(map<D3DRENDERSTATETYPE, DWORD> &renderState);
