...
void CVSInput9::on_add_node_child(StateDataNode* parent, StateDataNode* child) {}

void CVSInput9::on_remove_node_child(StateDataNode* parent, StateDataNode* child) {}

void CVSInput9::on_write_node_data(StateDataNode* node, size_t size, unsigned int offset) {}


void CVSInput9::on_added_controller(StateDataNode* node) {
...
void CVSInput9::on_removed_controller(StateDataNode* node) {
    if(node->get_id().name == NAME_DEVICE_9) {
...
void CVSInput9::on_assigned(ResourceAssignationTable* table, UsageId usage, ResourceId resource) {
    if(table->get_name() == NAME_ELEMENT_STREAM) {
        ...
        if(it_r != assigned_registers.end()) {
            ...
            GPUProxy::get_instance()->writeGPURegister(GPU_VERTEX_ATTRIBUTE_MAP, gpu_register, data);

            D3D_DEBUG( cout << "CVSINPUT9: Mapping vs input register "; )
            D3D_DEBUG( cout << gpu_register << " to stream " << gpu_stream << endl; )
            D3D_DEBUG( cout << "CVSINPUT9: Usage is "; )
        ...
        if(it_r != assigned_streams.end()) {
            ...
            ResourceId r;
            UsageId u = *it_r;
            r = AssignationsRegistry::get_table(NAME_ELEMENT_STREAM)->get_assigned_to(u);
            ...
            D3D_DEBUG( cout << "CVSINPUT9: Unmapped vs input register "; )
...
void CVSInput9::on_unassigned(ResourceAssignationTable* table, UsageId usage, ResourceId resource) {
    if(table->get_name() == NAME_ELEMENT_STREAM) {
